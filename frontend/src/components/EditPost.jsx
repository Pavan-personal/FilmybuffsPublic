...
export default function EditPost(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    post.title ? formData.append("title", post.title) : (count += 1);
    post.description
      ? formData.append("description", post.description)
      : (count += 1);
    post.attachment ? formData.append("file", post.attachment) : (count += 1);
    if (count === 3) {
      toast.error("fill atleast one field to edit!");
      return;
    }
    setHomeLoad(true);
    const response = await axios.put(
      `https://filmy-buffs-backend.vercel.app/recruiter/edit/${props.postId}`,
      formData,
      {
        headers: {
    ...
    <div className="dark:bg-[#082f49] bg p-5 flex justify-center items-center w-fit h-screen">
      <div className=" py-9 rounded-xl p-6">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 w-80 sm:grid-cols-2 sm:gap-6">
